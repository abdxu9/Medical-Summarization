...
Summary:"""
    ...
    def save_results(self, model_results: Dict[str, Dict]):
        """Save hyperparameter search results"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        results_file = f"{self.config.output_dir}/results/hyperparameter_results_{timestamp}.json"
        ...
        self._create_visualizations(model_results, timestamp)
    ...
    def run_optimization(self):
        ...
        for model_config in model_configs:
            ...
            if self.config.method == "random":
                results = self.random_search(model_config)
        ...
        # Return best parameters for each model
        best_params = {}
        for model_name, results in all_results.items():
            if 'best_trial' in results and results['best_trial']:
                best_params[model_name] = {
                    'params': results['best_trial']['params'],
                    'eval_loss': results['best_trial']['eval_loss'],
                    'metrics': results['best_trial'].get('metrics', {})
                }
