...
Models supported:
- medgemma-27b-text-it (with QLoRA)
...
class HyperparameterSpace:
    ...
    batch_size_choices: List[int] = field(default_factory=lambda: [8])  # Choices for batch size
...
class SearchConfig:
    ...
    num_train_epochs: int = 1       # Number of epochs for each trial
    save_strategy: str = "steps"
    gradient_checkpointing: bool = True
    max_memory_per_gpu: Optional[str] = "47GiB"  # For RTX 5090
    random_seed: int = 42
...
Summary:"""
    ...
    def sample_hyperparameters(self, rng: np.random.Generator) -> Dict[str, Any]:
        """Sample hyperparameters from the defined space using a dedicated RNG."""
        params = {
            "lora_rank": int(rng.integers(low=self.hp_space.lora_rank_min, high=self.hp_space.lora_rank_max, endpoint=True)),
            "lora_alpha": int(rng.integers(low=self.hp_space.lora_alpha_min, high=self.hp_space.lora_alpha_max, endpoint=True)),
            "lora_dropout": rng.uniform(low=self.hp_space.lora_dropout_min, high=self.hp_space.lora_dropout_max),
            "learning_rate": rng.uniform(low=self.hp_space.learning_rate_min, high=self.hp_space.learning_rate_max),
            "batch_size": int(rng.choice(self.hp_space.batch_size_choices)),
            "warmup_ratio": rng.uniform(low=self.hp_space.warmup_ratio_min, high=self.hp_space.warmup_ratio_max)
        }
        return params
    ...
    def evaluate_trial(self, model_config: ModelConfig, trial_params: Dict[str, Any]) -> Tuple[float, Dict[str, Any]]:
        ...
        num_update_steps_per_epoch = math.ceil(len(train_dataset_tokenized) / (trial_params["batch_size"] * gradient_accumulation_steps))
        total_training_steps = num_update_steps_per_epoch * self.config.num_train_epochs
        
        # Calculate evaluation steps based on evals_per_epoch
        if self.config.evals_per_epoch > 0:
            eval_steps = max(1, num_update_steps_per_epoch // self.config.evals_per_epoch)
        else: # Evaluate once at the end of the epoch if evals_per_epoch is 0 or less
            eval_steps = num_update_steps_per_epoch

        self.logger.info(f"Trial training for {self.config.num_train_epochs} epochs, equivalent to {total_training_steps} steps.")
        self.logger.info(f"Evaluation will occur every {eval_steps} steps.")
    ...
    def random_search(self, model_config: ModelConfig):
        ...
        for trial_idx in range(self.config.n_trials):
            trial_params = self.sample_hyperparameters(rng)
    ...
    def run_optimization(self):
        ...
        # Save the detailed hyperparameter search results
        self.save_hyperparameter_search_details(all_hyperparam_results)
        
        # Prepare the return value for the main() function
...
def main():
    ...
    parser.add_argument("--num_train_epochs", type=int, default=1, help="Number of epochs to train per trial.")
